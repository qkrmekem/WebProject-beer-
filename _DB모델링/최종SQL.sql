...
CREATE SEQUENCE t_article_SEQ
START WITH 1
INCREMENT BY 1;
/

CREATE OR REPLACE TRIGGER t_article_AI_TRG
BEFORE INSERT ON t_article 
REFERENCING NEW AS NEW FOR EACH ROW 
BEGIN 
    SELECT t_article_SEQ.NEXTVAL
    INTO :NEW.b_seq
    FROM DUAL;
END;
/

--DROP TRIGGER t_article_AI_TRG; /

--DROP SEQUENCE t_article_SEQ; /

COMMENT ON TABLE t_article IS '게시판'
/

COMMENT ON COLUMN t_article.b_seq IS '글 순번'
/

COMMENT ON COLUMN t_article.b_title IS '글 제목'
/

COMMENT ON COLUMN t_article.b_content IS '글 내용'
/

COMMENT ON COLUMN t_article.b_date IS '글 작성일자'
/

COMMENT ON COLUMN t_article.b_file IS '글 첨부파일'
/

COMMENT ON COLUMN t_article.m_id IS '글 작성자'
/

COMMENT ON COLUMN t_article.b_cnt IS '글 조회수'
/

ALTER TABLE t_article
    ADD CONSTRAINT FK_t_article_m_id FOREIGN KEY (m_id)
